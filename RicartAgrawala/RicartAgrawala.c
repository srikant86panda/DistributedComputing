...
void send_reply();			//Function to send reply messages
...
int main(int argc, char **argv)
{
	...
	FILE *file;  
	file = fopen("ricartAgrawala_config.txt", "r"); //Open the configuration file
...
//Prakash Code
	{
		pthread_create( &proc1[s], NULL, &processes, (void *)(intptr_t)s);	//Creating processes in site
	}
...
{
	int nodeseq;
	long pid; //use it for ripping the process to b sent to 
	struct message m;
	m = *((struct message *)msg);
	while(1)
	{
