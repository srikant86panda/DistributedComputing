...
//************************************************
//STUDENT NAME	: SRIKANT PANDA
//STUDENT ID	: 2016HT12588
...
void displayBuffer(){
    ...
    while(temp !=NULL){
        for(int p =-1;p<totalProcessor;p++){
            printf("-");
        }
        printf("\n");
        for(int p =0;p<totalProcessor;p++){
...
void checkBufferAndDeliver(struct sockaddr_in peerAddr){
    ...
    while(temp !=NULL && !(temp->isDelivered)){
        if(pclock[temp->rclock[totalProcessor]] == temp->rclock[temp->rclock[totalProcessor]] -1){
            int deliver =1;
            for(int c=0;c<totalProcessor;c++){
                if(c!= temp->rclock[totalProcessor]){
            ...
            if(deliver){
                printf("\nDelivered BUffered Message Which Was Received From:-  %s(%d),  Msg :- %d \n", inet_ntoa(peerAddr.sin_addr), ports[temp->rclock[totalProcessor]], temp->rclock[temp->rclock[totalProcessor]]);
                temp->isDelivered =1;
                for(int c=0;c<totalProcessor;c++){
...
void handleDelivery(int *rclock,struct sockaddr_in peerAddr){
    ...
    if(pclock[*(rclock +totalProcessor)] == *(rclock +(*(rclock +totalProcessor))) -1){
        int deliver =1;
        for(int c=0;c<totalProcessor;c++){
            if(c!= (*(rclock+totalProcessor))){
        ...
        if(deliver){
            printf("\nDelivered Message From:-  %s(%d),  Msg :- %d \n", inet_ntoa(peerAddr.sin_addr), ports[*(rclock+totalProcessor)], *(rclock+ (*(rclock+totalProcessor))));
            for(int c=0;c<totalProcessor;c++){
...
void sendBCMsg(char * msgsend){
    ...
    int sent=0;
    for(int p=processId+1;sent<totalProcessor-1;p++){
        if ((sendSock = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0)
            DieWithError("socket () failed\n") ;
        memset(&peerAddr, 0, sizeof(peerAddr));
        peerAddr.sin_family = AF_INET;
        peerAddr.sin_addr.s_addr = inet_addr(peerlP);
        peerAddr.sin_port = htons(ports[ p % totalProcessor]);
        sleep(channelDelay[p % totalProcessor]);
...
void initChannelDelay(){
    ...
    int chupdated=1;
    for(int ch=processId+1;chupdated<totalProcessor;ch++){
        channelDelay[ ch % totalProcessor ] = delay;
        delay+=3;
        chupdated++;
    }
    printf("Channel Delays to Process %d to %d ----> ",0,totalProcessor-1);
    for(int ch =0 ;ch<totalProcessor;ch++){
...
{
    printf("Enter Total Number Of Processes (max 99):\n");
    scanf("%d",&totalProcessor);
    printf("Enter Process Id of Current Process (should be 0 to %d)\n",totalProcessor-1);
    scanf("%d",&processId);
	printf("Selected Process ID: %d\n",processId);
    
    for(int p=0;p<totalProcessor;p++){
        printf("Enter (Procees-%d) Port To Recevice Brodacast Message\n",p);
        scanf("%d",&ports[p]);
    }
    pclock[totalProcessor] = processId;
